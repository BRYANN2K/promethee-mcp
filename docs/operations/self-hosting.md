...
The public `v0.1.2` source release has a tested GitHub Release+npx local stdio composition, a loopback personal HTTP composition, a validated publisher-RPC Supabase composition, a separately built passwordless web package, and a checked-in single-user Docker Compose/Caddy deployment candidate. It is not yet a verified VPS deployment: the release has not been exercised on a clean host, key rotation and rollback are unproven, and live Promethee compatibility remains unofficial and version-sensitive.
...
```bash
npx -y --package=https://github.com/BRYANN2K/promethee-mcp/releases/download/v0.1.2/promethee-mcp-0.1.2.tgz -- prometheemcp
```
...
The release archive is immutable by convention and has been exercised directly through `npx`. Verify the release checksum before introducing it into a managed environment. Installing from a Git ref is not the supported `v0.1.2` path because npm `10.9.2` fails its Git packaging step before this package starts.
